      ...
      <nav className="sticky top-0 z-20 border-b border-white/5 bg-brand-bg/80 backdrop-blur-sm">
        <div className="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link href="/platform/kdp" className="text-sm font-medium text-brand-cream hover:text-brand-gold transition-colors">Tools</Link>
            <Link href="/platform/kdp" className="text-sm font-medium text-brand-cream hover:text-brand-gold transition-colors">Amazon KDP</Link>
          </div>
        </div>
      </nav>
      ...
      <main className="mx-auto max-w-4xl px-6 py-12">
        ...
        <div className="rounded-xl border border-brand-cardHover bg-brand-card p-6 mb-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block font-sans text-sm font-medium text-brand-cream mb-2">Page count</label>
              <input
                type="number"
                min={MIN_PAGES}
                max={MAX_PAGES}
        ...
        <div className="rounded-xl border border-brand-cardHover bg-brand-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full font-sans text-sm">
              <thead>
                <tr className="border-b border-brand-cardHover">
                  <th className="text-left py-3 px-4 text-brand-cream font-medium">Trim size</th>
                  <th className="text-right py-3 px-4 text-brand-cream font-medium">Print cost</th>
                  <th className="text-right py-3 px-4 text-brand-cream font-medium">Royalty/sale</th>
                </tr>
              </thead>
              <tbody>
                {rows.map(({ trim, printCost, royalty }) => (
                  <tr key={trim.id} className="border-b border-brand-cardHover last:border-0">
                    <td className="py-3 px-4 text-brand-cream">{trim.name}</td>
                    <td className="py-3 px-4 text-right text-brand-muted">${printCost.toFixed(2)}</td>
                    <td className="py-3 px-4 text-right text-brand-gold font-semibold">${royalty.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
