...
`);
  ...
  // Re-fetch report when annotation completes so annotatedPdfDownloadUrl is populated
  useEffect(() => {
    if (annotatedReady && !report?.annotatedPdfDownloadUrl) {
      loadReport();
    }
  }, [annotatedReady, report?.annotatedPdfDownloadUrl, loadReport]);
  ...
  // On every checker page load: silently call annotate-local.
  // If the annotation version is current, the server returns the cached URL instantly (no work done).
  // If the version is stale or no annotated PDF exists yet, it re-annotates and stores the new URL.
  useEffect(() => {
    if (!isChecker || !id) return;
              ...
              <>
                ...
                <div className="mt-6 pt-6 border-t border-m2p-border/50 no-print">
                  ...
                  >
                    ...
                    {isChecker && hasActionablePageIssues && !annotatedError && (
                      <div className="mt-2 mb-2">
                        {hasAnnotatedDownload && report?.annotatedPdfDownloadUrl ? (
                          <a
                            href={report.annotatedPdfDownloadUrl + "?v=16"}
                          ...
                          <p className="text-xs text-m2p-muted text-center animate-pulse py-2">
