...
/** All tools; platforms reference by id. */
...
/** Platform hub: each box = logo + tagline + that platform's tools. */
const PLATFORMS: { id: string; name: string; tagline: string; toolIds: string[] }[] = [
...
function getToolsForPlatform(toolIds: string[]): Tool[] {
  return toolIds.map((id) => ALL_TOOLS.find((t) => t.id === id)).filter(Boolean) as Tool[];
}

/** Platform box: logo area + tagline + tools (or Coming soon). */
function PlatformBox({
  platform,
  tools,
}: {
  platform: { id: string; name: string; tagline: string };
  tools: Tool[];
}) {
  const initial = platform.name.charAt(0);
  return (
    <section className="rounded-xl border border-brand-cardHover bg-brand-card overflow-hidden mb-10">
      {/* Header: logo slot + name + tagline */}
      <div className="flex flex-wrap items-center gap-4 p-6 border-b border-white/5">
        ...
        <div>
          <h3 className="font-bebas text-2xl tracking-wide text-brand-cream">{platform.name}</h3>
          <p className="font-sans text-sm text-brand-muted mt-0.5">{platform.tagline}</p>
        </div>
      </div>
      {/* Tools grid or Coming soon */}
      <div className="p-6">
        {tools.length > 0 ? (
          ...
          <p className="font-sans text-brand-muted text-center py-6">Coming soon. Sign up on the Formatter page to be notified.</p>
        )}
      </div>
    </section>
  );
