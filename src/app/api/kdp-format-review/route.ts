...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = `You are a professional KDP (Kindle Direct Publishing) formatter. Review the manuscript for KDP interior requirements. Be specific and actionable. Output only valid JSON, no markdown or extra text.`;
...
1. "sections" (array of objects). Each object has: "title" (string), "issues" (array of strings, optional), "recommendations" (array of strings, optional), "content" (string, optional brief notes). Use these section titles exactly:
   - "Front matter & back matter" (order: title page, copyright, dedication, TOC, then body; back matter: about author, etc. Flag wrong order or missing elements.)
   - "Margins & gutter" (inner margin at least 0.5" + gutter; KDP page count determines recommended gutter.)
   - "Paragraphs & indentation" (first line indent or block spacing, consistency; avoid no-indent with no space between.)
   - "Spacing" (consistent line spacing; avoid single orphan lines at top/bottom of pages.)
   - "Widows & orphans" (avoid single word on last line of paragraph or single line at top/bottom of page; recommend "keep with next" for headings.)
   - "Headings" (consistent hierarchy; chapter starts on new page; no heading at bottom of page with no following text.)
   - "Page breaks" (chapter starts on recto where expected; blank pages where needed.)
   - "Lists" (consistent indentation and spacing.)
   - "Fonts & typography" (KDP allows embedded fonts; 11–12 pt body typical; avoid too many fonts.)
   - "Images & graphics" (if mentioned or implied: recommend 300 DPI for interior images; placement and captions.)
   - "KDP rules & gotchas" (bleed only if needed; trim size consistency; no content in bleed area; PDF must be print-ready.)

2. "summary" (string): 2–4 sentences overall assessment. Be direct: is it close to KDP-ready or does it need clear fixes?

3. "topActions" (array of strings): 3–5 prioritized actions the author should fix first. Be specific (e.g. "Set inner margin to at least 0.75\" for 6×9" or "Start each chapter on a new page"). These are the most important fixes to avoid KDP rejection or poor print quality.

4. "kdpReadiness" (string): One short line, e.g. "Close; fix gutter and chapter page breaks" or "Good; minor spacing tweaks recommended" or "Needs work; fix margins and front matter order." Make it shareable and clear.
