...
export async function POST(request: NextRequest) {
  try {
    const rateLimited = await enforceDurableRouteLimit({
      req: request,
      routeKey: "ai:keyword-research-docx",
      maxRequests: 12,
      windowSeconds: 10 * 60,
    });
    ...
    if (isPdf) return NextResponse.json({ error: "PDF not supported", message: "This tool accepts .docx files only." }, { status: 400 });
    const isDocx = name.endsWith(".docx") || f.type.includes("wordprocessingml");
    if (!isDocx) return NextResponse.json({ error: "Unsupported format", message: "This tool accepts .docx files only." }, { status: 400 });
